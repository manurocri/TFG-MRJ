...
if(isset($_POST["signup-submit"])){
	 ...
	 if(empty($username) || empty($email) || empty($password) || empty($passwordRepeat)){
	 	header("Location: ../php/signup.php?error=emptyfields&uid=".$username."&email=".$email);
	 	exit();
	 	...
	 	header("Location: ../php/signup.php?error=invaliduid&email=".$email);
	 	exit();
	 }elseif ($password !== $passwordRepeat) {
	 	header("Location: ../php/signup.php?error=passwordcheck&uid=".$username."&email=".$email);
	 	exit();
	 }else{
	 	...
	 	if (!mysqli_stmt_prepare($stmt,$sql)) {
	 		...
	 		if ($resultCheck > 0) {
	 			header("Location: ../php/signup.php?error=usertaken&email=".$email);
	 		exit();
	 		}else{
